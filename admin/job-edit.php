<?php
// admin/job-edit.php — Editar cualquier oferta de trabajo como administrador
...
$stmtJob = $pdo->prepare("SELECT * FROM jobs WHERE id = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '') {
        ...
        if ($salaryMinVal !== null && $salaryMaxVal !== null && $salaryMinVal > $salaryMaxVal) {
            ...
            header('Location: ' . BASE_URL . '/admin/job-detail.php?id=' . $jobId . '&edited=1');
            exit;
        }
    }
}
...
<a href="<?= BASE_URL; ?>/admin/job-detail.php?id=<?= $jobId; ?>" class="btn-link" style="display:inline-block; margin-bottom:1rem;">
    ← Back to details
</a>

<section class="auth-box">
    <h1>Edit job listing</h1>
    <p>Changes are applied while keeping the current status of the job listing.</p>

    <?php if ($error !== ''): ?>
        <div class="alert alert-error"><?= htmlspecialchars($error); ?></div>
    <?php endif; ?>

    <form method="post" action="<?= BASE_URL; ?>/admin/job-edit.php?id=<?= $jobId; ?>" class="auth-form" novalidate id="form-job">
        ...
        <a href="<?= BASE_URL; ?>/admin/job-detail.php?id=<?= $jobId; ?>" class="btn-link btn-cancel">Cancel</a>
    </form>
</section>
