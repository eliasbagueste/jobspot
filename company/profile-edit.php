    ...
    if ($legalName === '' || $brandName === '') {
        ...
        if (isset($_FILES['logo']) && $_FILES['logo']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($mimeType, $allowedTypes)) {
                $error = 'The logo must be an image (JPG, PNG, WebP or GIF).';
            } elseif ($_FILES['logo']['size'] > 2 * 1024 * 1024) {
                $error = 'The logo cannot exceed 2 MB.';
            } else {
                ...
                if (move_uploaded_file($_FILES['logo']['tmp_name'], $uploadsDir . $filename)) {
                    ...
                    $error = 'Could not save the logo. Check the permissions on the uploads/logos/ folder.';
                }
            }
        }
...
<section class="auth-box">
    ...
    <form method="post" action="<?= BASE_URL; ?>/company/profile-edit.php"
        ...
        <div class="form-group">
            <label for="tax_id">Tax ID (CIF/NIF)</label>
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" rows="5"
                      ...
                      placeholder="Tell us what your company does..."><?= htmlspecialchars($company['description'] ?? ''); ?></textarea>
        </div>
        ...
        <div class="form-group">
            ...
            <?php if (!empty($company['logo_path'])): ?>
                <div style="margin-bottom:0.5rem;">
                    ...
                    <p style="font-size:0.82rem; color:#64748b; margin:0.25rem 0 0;">Current logo. Upload a new one to replace it.</p>
                </div>
            <?php endif; ?>
            <input type="file" id="logo" name="logo" accept="image/jpeg,image/png,image/webp,image/gif"
                   style="padding:0.3rem 0;">
            <span style="font-size:0.8rem; color:#94a3b8;">JPG, PNG, WebP or GIF · max. 2 MB</span>
        </div>

        <button type="submit" class="btn-primary">Save changes</button>
        <a href="<?= BASE_URL; ?>/company/index.php" class="btn-link btn-cancel">Cancel</a>
    </form>
    ...
    document.getElementById('form-profile').addEventListener('submit', function (e) {
        ...
        if (!legalName.value.trim())
            fieldError(legalName, 'Legal name is required.');

        if (!brandName.value.trim())
            fieldError(brandName, 'Trading name is required.');

        if (website.value.trim() && !website.value.trim().startsWith('http'))
            fieldError(website, 'Enter a valid URL (must start with http:// or https://).');

        if (logo.files.length > 0) {
            const file = logo.files[0];
            const allowed = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
            if (!allowed.includes(file.type))
                fieldError(logo, 'The logo must be an image (JPG, PNG, WebP or GIF).');
            else if (file.size > 2 * 1024 * 1024)
                fieldError(logo, 'The logo cannot exceed 2 MB.');
        }
